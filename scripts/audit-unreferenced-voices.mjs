...
const records = candidates.map((candidate) => {
  ...
  const isFormalRuntimeAsset = runtimeReference && manifestReference;
  return {
    candidate,
    entry,
    node,
    ...
    duplicateFiles,
    relatedCurrentEntry,
    isFormalRuntimeAsset,
  };
});

const missingReferences = records.filter((record) => !record.manifestReference || !record.runtimeReference);
if (checkOnly) {
  if (records.length !== 32) throw new Error(`Expected 32 m02 WAV files; found ${records.length}.`);
  if (missingReferences.length) throw new Error(`m02 audit found ${missingReferences.length} file(s) without both manifest and runtime references.`);
...
const details = records.map((record, index) => {
  ...
  return [
    ...
    row("Missing-coverage value", record.isFormalRuntimeAsset ? "No. The mapped node already has this exact formal WAV." : "Needs investigation before any cleanup."),
    row("Decision", record.isFormalRuntimeAsset ? "Keep. It is a referenced formal runtime asset; deleting it would break voice coverage." : "Keep pending investigation."),
  ].join("\n");
}).join("\n\n");
...
console.log(`Wrote ${path.relative(root, outputPath)}. files=${records.length}; referenced=${records.filter((item) => item.isFormalRuntimeAsset).length}; removable=0`);
