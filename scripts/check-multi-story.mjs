...
assert(canonicalEndingCount === 0 || canonicalEndingCount === 4 || canonicalEndingCount === 8, "Canonical dormitory runtime must export zero, four, or all eight formal endings according to the active Draft phase.");
if (canonicalEndingCount === 8) {
  ...
  assert(namefloor?.nodes?.nf06_050?.nextNodeId === "nf07_001", "Canonical dormitory Chapter 6 hook must advance into Chapter 7.");
  assert(namefloor?.nodes?.nf07_decision_entry?.nextNodeId === "nf_end_resolve", "Canonical dormitory Chapter 7 must enter the unified ending resolver.");
  if (canonicalEndingCount === 4) {
    assert(namefloor?.nodes?.nf_end_draft_stop?.type === "chapter-ending", "Unimplemented ending candidates need a Draft stop node.");
  }
  assert(Object.keys(namefloor?.nodes || {}).length >= 320, "Canonical dormitory Chapter 1-6 Draft phase is unexpectedly incomplete.");
  for (const chapterId of ["namefloor_chapter_02", "namefloor_chapter_03", "namefloor_chapter_04", "namefloor_chapter_05", "namefloor_chapter_06", "namefloor_chapter_07"]) {
