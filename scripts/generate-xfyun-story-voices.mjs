...
const roles = {
  Narrator: { roleId: "narrator", label: "旁白", candidates: ["x6_pangbainv1_pro", "x6_lingxiaoyun_pro", "x5_lingxiaoyue_flow"], speed: 45, pitch: 47, volume: 48 },
  ...
  旁白: { aliasOf: "Narrator" },
  林舟: { roleId: "linzhou", label: "林舟", candidates: ["x6_lingxiaoxuan_pro", "x6_lingxiaoyue_flow", "x6_lingyuyan_flow"], speed: 47, pitch: 49, volume: 49 },
  许知晚: { roleId: "xuzhiwan", label: "许知晚", candidates: ["x6_ganliannvxing_pro", "x6_lingxiaoli_pro", "x6_lingxiaoxue_pro"], speed: 44, pitch: 45, volume: 48 },
  许知夏: { roleId: "xuzhixia", label: "许知夏", candidates: ["x6_lingxiaoyue_pro", "x6_lingxiaoyun_pro", "x6_lingxiaoxuan_flow"], speed: 44, pitch: 53, volume: 47 },
  "许知夏的声音": { aliasOf: "许知夏" },
  周屿: { roleId: "zhouyu", label: "周屿", candidates: ["x6_lingfeiyi_pro", "x6_lingfeihan_pro", "x6_bokenansheng_pro"], speed: 44, pitch: 42, volume: 48 },
  陈妍: { roleId: "chenyan", label: "陈妍", candidates: ["x6_lingyufei_pro", "x6_lingxiaoshan_pro", "x6_lingyuzhao_flow"], speed: 49, pitch: 50, volume: 50 },
  女人: { roleId: "woman_at_door", label: "门外女人", candidates: ["x6_lingxiaoyun_pro", "x6_lingxiaoli_pro", "x6_lingyuyan_flow"], speed: 43, pitch: 48, volume: 47 },
  房东老太: { roleId: "landlady", label: "房东老太", candidates: ["x6_huifangnv_pro", "x6_lingxiaoshan_pro", "x6_lingyuzhao_flow"], speed: 44, pitch: 40, volume: 49 },
};
...
function readCachedCasting() { try { const context = { window: {} }; vm.runInNewContext(fs.readFileSync(path.join(root, "assets", "voice-casting-manifest.js"), "utf8"), context, { filename: "voice-casting-manifest.js" }); return context.window.SECOND_LIFE_VOICE_CASTING?.storyId === story.id ? context.window.SECOND_LIFE_VOICE_CASTING.roles || {} : {}; } catch { return {}; } }
function writeAudit(targets, manifest) { const rows = targets.map((target) => { const e = manifest.entries[target.id]; return `| ${story.id} | ${target.id} | ${target.chapterId} | ${target.speaker} | ${target.displayText.replace(/\|/g, "\\|")} | 是 | ${target.roleId} | ${e?.status || "missing"} | ${story.prefix}_${target.id}__${target.roleId}.wav | ${e?.status === "generated" ? "是" : "否"} | ${e?.textHash !== hash(target.spokenText) ? "是" : "否"} |`; }); fs.writeFileSync(path.join(root, "docs", "VOICE_COVERAGE_AUDIT.md"), `# 剧情配音审计\n\n仅将正式旁白、角色对白、广播、录音、系统叙事、推理题干及结局正文计入覆盖。\n\n| storyId | nodeId | chapterId | speaker | 显示文字 | 需要配音 | 角色 | 当前状态 | 计划文件名 | 已有音频 | 需要重生 |\n| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |\n${rows.join("\n")}\n`); }
function writeRuntime(manifest) { const storiesOut = { [story.id]: { nodes: {}, endings: {}, cues: {} } }; for (const entry of Object.values(manifest.entries)) { if (entry.status !== "generated") continue; const key = entry.kind === "ending" ? "endings" : entry.kind === "broadcast-cue" ? "cues" : "nodes"; storiesOut[story.id][key][entry.cueId || entry.id] = { path: entry.webPath, roleId: entry.roleId, textHash: entry.textHash }; } fs.writeFileSync(path.join(root, "assets", "voice-runtime-manifest.js"), `(function () {\n  \"use strict\";\n  window.SECOND_LIFE_VOICE_MANIFEST = ${JSON.stringify({ version: 1, stories: storiesOut }, null, 2)};\n})();\n`); }
...
async function main() {
  ...
  const roleIds = [...new Set(targets.map((target) => target.roleId))]; const cast = {}; const used = new Set(); const cachedCasting = readCachedCasting();
  for (const roleId of roleIds) { const cached = cachedCasting[roleId]; if (cached?.apiTestStatus === "authorised" && cached.vcn && !used.has(cached.vcn)) { cast[roleId] = cached; used.add(cached.vcn); } }
  ...
  fs.writeFileSync(path.join(root, "assets", "voice-casting-manifest.js"), `(function () { window.SECOND_LIFE_VOICE_CASTING = ${JSON.stringify({ provider: "xfyun-super-smart-tts-webapi", storyId: story.id, roles: cast }, null, 2)}; })();\n`);
  fs.writeFileSync(path.join(root, "docs", "VOICE_CASTING.md"), `# 剧情配音选角\n\n提供商：科大讯飞超拟人语音合成 WebAPI。每个角色通过短文本探测后使用独立授权 vcn。\n\n${Object.values(cast).map((item) => `- ${item.label}: \`${item.vcn}\`，语速 ${item.speed}，语调 ${item.pitch}，音量 ${item.volume}。`).join("\n")}\n`);
