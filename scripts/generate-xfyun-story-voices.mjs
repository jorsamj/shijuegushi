...
// Legacy-only helper kept for rollback audits. Formal runtime voice delivery is
// generated by generate-volcengine-story-voices.mjs and promoted through the
// Volcengine voice manifests.
...
async function main() {
  ...
  fs.writeFileSync(path.join(root, "docs", "VOICE_CASTING_XFYUN_LEGACY.md"), `# Legacy XFYUN Voice Casting\n\nProvider: XFYUN Super Smart TTS WebAPI. This file is generated only by the legacy helper and is not the formal runtime voice-casting document.\n\n${Object.entries(registry.stories).map(([storyId, entry]) => `## ${storyId}\n\n${Object.values(entry.roles || {}).map((item) => `- ${item.label}: \`${item.vcn}\`, speed ${item.speed}, pitch ${item.pitch}, volume ${item.volume}.`).join("\n")}`).join("\n\n")}\n`);
