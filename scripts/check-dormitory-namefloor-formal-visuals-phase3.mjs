...
const masterId = "char_zhouchaoyang_master_upper_neutral";
const stateIds = ["analyzing", "suspicious", "guarded", "evidence_shaken", "angry_resolute", "regret_exhausted"];
...
assert(master?.assetType === "character-master" && master.characterId === "zhouchaoyang", "Zhou Chaoyang master metadata is invalid");
assert(typeof master.path === "string" && fs.existsSync(path.join(root, master.path)), "Zhou Chaoyang master file is missing");

for (const stateId of stateIds) {
  const asset = assets.find((entry) => entry.assetType === "character-state" && entry.characterId === "zhouchaoyang" && entry.stateId === stateId);
  assert(asset, `Zhou Chaoyang state is not registered: ${stateId}`);
  assert(typeof asset.path === "string" && asset.path.trim().length > 0, `Zhou Chaoyang state path is empty: ${stateId}`);
  assert(asset.sourceMaster === master.path, `Zhou Chaoyang state must use the registered master: ${stateId}`);
  assert(asset.model === "gpt-image-2", `Zhou Chaoyang state must record gpt-image-2: ${stateId}`);
  assert(asset.width === 1024 && asset.height === 1536 && asset.alpha === true, `Zhou Chaoyang state metadata is invalid: ${stateId}`);
  assert(asset.runtimeIntegrated === false, `Zhou Chaoyang state cannot be runtime integrated in phase 3: ${stateId}`);
  const absolute = path.join(root, asset.path);
  assert(fs.existsSync(absolute) && fs.statSync(absolute).size > 0, `Zhou Chaoyang state is missing or empty: ${stateId}`);
  const png = decode(fs.readFileSync(absolute));
  assert(png.width === 1024 && png.height === 1536 && hasRealAlpha(png), `Zhou Chaoyang state needs 1024x1536 real alpha PNG: ${stateId}`);
}

console.log("phase-3 formal visuals OK: 6 approved Zhou Chaoyang state images");
