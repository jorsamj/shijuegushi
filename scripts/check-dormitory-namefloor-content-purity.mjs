...
function checkPackage(packageEntry, isCanonical) {
  ...
  // Ending bodies are selected by the deterministic resolver rather than a visible choice edge.
  if (nodes.nf_end_resolve) {
    const endingStarts = Object.entries(nodes)
      .filter(([, node]) => node.endingRouteId || node.nodeId === "nf_end_draft_stop")
      .map(([nodeId]) => nodeId);
    for (const startNodeId of endingStarts) {
      const routeData = { ...data, script: { ...(data.script || {}), startNodeId } };
      for (const nodeId of reachableNodes(routeData)) reachable.add(nodeId);
    }
  }
