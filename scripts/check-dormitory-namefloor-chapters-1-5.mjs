...
function walkDefaultRoute(data, startNodeId) {
  ...
  for (let step = 0; step < 420; step += 1) {
    ...
    route.push(node);
    if ((node.type === "chapter-ending" && !node.nextNodeId) || node.resolveEnding === true) return route;
...
assert.equal(defaultRoute.at(-1)?.nodeId, "nf_end_resolve", "Current formal runtime must reach the unified ending resolver.");
assert.equal(defaultRoute.at(-1)?.resolveEnding, true, "The formal route must finish at the deterministic resolver.");
