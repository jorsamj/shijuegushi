(function () {
  ...
  const scenes = {
    ...
    namefloor_blank_contacts: scene("空白通讯录", "nf-scene-blank-contacts", "phone-interface-closeup", "dormitory_phone_silence", "single-vibration", ["name-erasure", "signal-static"], ["group:members", "system:save"], "深夜宿舍床位旁的手机通讯录，林峰条目为空，班群四十二人不变，屏幕冷光照出手指。", "phone"),
    namefloor_break_roster: scene("砸碎名册", "nf-scene-roster-break", "low-angle-action", "dormitory_archive_air", "staple-snap", ["paper-scatter", "ember-fall"], ["system:documents"], "校长室内固定人数名册被砸断装订钉，散页向上翻起，两个林峰的档案在背景错位。", "roster"),
    namefloor_changed_photo: scene("被替换的合照", "nf-scene-changed-photo", "phone-photo-closeup", "dormitory_phone_silence", "future-message-tone", ["photo-replace", "timestamp-glitch"], ["private:message", "system:gallery"], "手机上的班级合照中林峰位置被相同面孔覆盖，四十二人群人数保持不变，未来时间戳清晰可读。", "phone"),
    namefloor_copy_dorm: scene("复制的1107", "nf-scene-copy-dorm", "dormitory-two-plane", "dormitory_fourth_floor_air", "bedframe-creak", ["double-exposure", "curtain-shift"], ["group:members"], "不存在四楼的复制1107，宋明坐在原床位，手腕勒痕可见，男生宿舍床铺与门位保持连续。", "room"),
    ...
    namefloor_dual_rules: scene("两套规则", "nf-scene-dual-rules", "desk-document-closeup", "dormitory_manager_air", "paper-turn", ["ink-wet", "rule-underline"], ["system:rules"], "宿管宿舍桌面上学生规则与宿管规则并排，羽毛和笔记压住纸角，男生宿舍环境保持低光。", "rules"),
    ...
    namefloor_ending_breakfast: scene("清晨早餐店", "nf-scene-ending-breakfast", "dawn-tableau-vertical", "dormitory_dawn_air", "cup-settle", ["dawn-light", "contact-restore"], ["group:main"], "校外早餐店刚开门，林峰、周朝阳、谷雨与宋明在疲惫中确认彼此仍在，手机班群恢复可读。", "exit"),
    namefloor_ending_song_guard: scene("宋明留下的门", "nf-scene-ending-song-guard", "door-light-vertical", "dormitory_exit_air", "door-close", ["door-seal", "feather-split"], ["private:main"], "复制宿舍出口前，同行宋明隔着将要合上的门望向林峰，手机头像在白光里短暂保留。", "door"),
    namefloor_ending_guyu_voice: scene("无联系人语音", "nf-scene-ending-guyu-voice", "phone-vacancy-vertical", "dormitory_dawn_air", "unlisted-voice", ["contact-erase", "portrait-vacancy"], ["private:main"], "清晨的林峰握着手机，无联系人语音波形与合照空位并置，谷雨的姓名已从记录中淡去。", "phone"),
    namefloor_ending_betrayal_message: scene("迟到的消息", "nf-scene-ending-betrayal", "notification-closeup", "dormitory_exit_air", "late-notification", ["delayed-message", "signal-return"], ["private:main"], "宿舍楼外的黎明中，林峰手机亮起被删除账号的迟到消息，四楼窗口在远处收缩。", "phone"),
    namefloor_ending_all_present_notice: scene("全员到齐的公告", "nf-scene-ending-all-present", "notice-and-roster-vertical", "dormitory_dawn_air", "notice-rustle", ["row-rewrite", "cabinet-flicker"], ["system:main"], "清晨公告、四人宿舍名册与班群记录全部正确，却有一张熟悉面孔在合照里迟半拍眨眼。", "roster"),
    namefloor_ending_second_linfeng: scene("第二个林峰", "nf-scene-ending-second-linfeng", "dual-account-vertical", "dormitory_dawn_air", "account-ring", ["mirror-offset", "contact-blank"], ["system:main"], "两个外貌相同的男学生站在宿舍楼外，一部手机拥有完整合法资料，另一部只显示用户不存在。", "figures"),
    namefloor_ending_fifth_shadow: scene("还有一个人", "nf-scene-ending-fifth-shadow", "morning-shadow-vertical", "dormitory_dawn_air", "extra-footstep", ["sign-erasure", "shadow-extra"], ["group:main"], "四名学生离开宿舍楼，地面却有第五道不同步的影子；班群人数始终不变。", "exit"),
    namefloor_ending_midnight_loop: scene("不要忘记你的名字", "nf-scene-ending-midnight-loop", "midnight-phone-vertical", "dormitory_phone_silence", "send-pulse", ["avatar-lock", "time-rewind", "name-erasure"], ["private:main"], "黑色头像占据林峰原班群位置，零点短信停在发送前；不存在自动重开的循环。", "phone"),
    namefloor_endless_floor: scene("无名层走廊", "nf-scene-endless-floor", "corridor-vanishing-point", "dormitory_fourth_floor_air", "fluorescent-buzz", ["sign-erasure", "depth-loop"], ["private:message"], "不存在的四楼复制十一楼走廊，铁门排列到竖幅深处，门牌姓名全部缺字。", "corridor"),
    namefloor_era_doors: scene("不同时代的门", "nf-scene-era-doors", "layered-door-depth", "dormitory_fourth_floor_air", "old-door-knock", ["era-crossfade", "door-breath"], ["group:message"], "同一条男生宿舍走廊的门依次打开，门内叠映不同年代的1107与被困男学生。", "doors"),
    ...
    namefloor_feather_debt: scene("羽毛的代价", "nf-scene-feather-debt", "hand-and-feather-closeup", "dormitory_manager_air", "feather-scorch", ["ink-vein", "feather-ash"], ["system:documents"], "吴阿姨在三楼走廊伸手索回白羽毛，持有者手腕浮现墨线，绿色马甲保持真实老旧质地。", "feather"),
    ...
    namefloor_fixed_count_archives: scene("固定人数档案", "nf-scene-fixed-count", "roster-and-lockers", "dormitory_archive_air", "roster-scratch", ["row-rewrite", "cabinet-flicker"], ["system:documents"], "校长室四组档案柜围住自动改写的学生名册，每恢复一个姓名便有另一行被挤出。", "roster"),
    ...
    namefloor_green_no_answer: scene("绿色马甲未作答", "nf-scene-green-no-answer", "doorway-aftermath", "dormitory_manager_air", "key-drop", ["key-glint", "ink-transfer"], ["system:documents"], "旋转铁门合上后，绿色马甲宿管留下钥匙与录音，羽毛污染沿掌心墨线转移。", "door"),
    namefloor_green_vest: scene("绿色马甲宿管", "nf-scene-green-vest", "manager-approach", "dormitory_manager_air", "cloth-step", ["green-approach", "feather-react"], ["system:documents"], "三楼或四楼走廊尽头，可信但不绝对安全的绿色马甲吴阿姨先观察羽毛，再观察学生姓名。", "manager"),
    ...
    namefloor_manager_room: scene("三楼宿管宿舍", "nf-scene-manager-room", "room-investigation-vertical", "dormitory_manager_air", "clock-tick", ["drawer-open", "moon-glow"], ["system:members", "system:message"], "三楼宿管宿舍内有桌、左抽屉、学生名单、撕页日记与异常窗，所有陈设属于中国大学男生宿舍体系。", "room"),
    namefloor_manager_threshold: scene("宿管门口", "nf-scene-manager-threshold", "half-open-door-vertical", "dormitory_manager_air", "hinge-breath", ["door-gap", "phone-time-slip"], [], "三楼宿管宿舍门半掩，门缝无人，林峰、周朝阳和谷雨站在门外确认楼层与时间异常。", "door"),
    ...
    namefloor_name_feast: scene("姓名被吞食", "nf-scene-name-feast", "victim-doorway", "dormitory_fourth_floor_air", "distant-chew", ["collar-erasure", "faceless-rise"], ["system:rules"], "无名层门缝前一名男学生五官尚在而姓名正从衣领、手机和记忆里剥落，避免血腥特写。", "door"),
    ...
    namefloor_successor_door: scene("接任者铁门", "nf-scene-successor-door", "scanning-door-vertical", "dormitory_manager_air", "scanner-sweep", ["name-scan", "shadow-break"], ["system:documents"], "没有把手的旋转铁门以冷光扫描姓名，门后人物呼吸连续而影子中断，暗示接任代价。", "door"),
    namefloor_torn_diary: scene("撕页宿管日记", "nf-scene-torn-diary", "diary-and-notes", "dormitory_manager_air", "paper-rustle", ["page-lift", "ink-fade"], ["private:message"], "撕页日记与周朝阳旧笔记并置在宿管桌上，手写生活细节比电子扫描更可靠。", "diary"),
    namefloor_two_linfengs: scene("复制宿舍的两个林峰", "nf-scene-two-linfengs", "bunk-bed-confrontation", "dormitory_fourth_floor_air", "curtain-slide", ["identity-split", "screen-denial"], ["system:rules", "system:system"], "复制1107上铺帘子滑开，合法林峰坐起，玩家林峰手机显示用户不存在，二人均为男性大学生。", "figures"),
    namefloor_two_songmings: scene("两个宋明", "nf-scene-two-songmings", "phone-and-door-depth", "dormitory_fourth_floor_air", "duplicate-ringtone", ["call-overlap", "memory-flash"], ["private:incoming-call"], "同行宋明拿着响铃手机，复制宿舍深处另一名宋明的声音传来，两人共享旧记忆而拥有不同当夜经历。", "phone"),
    namefloor_wu_old_accounts: scene("历任宿管合同", "nf-scene-wu-accounts", "contract-wall-vertical", "dormitory_manager_air", "key-split", ["signature-fade", "photo-wall-shift"], ["system:documents"], "历任宿管照片墙后藏三年合同，吴阿姨签名逐年缺字，旧钥匙被掰开交给林峰。", "contracts"),
  };
  ...
  window.DORMITORY_NAMEFLOOR_ASSET_MAP = {
    ...
    scenes,
    phoneViews: {
      "group:main": { title: "恢复的班级群", layout: "portrait-chat-thread", anomaly: "林峰重新回到成员位，现场人数与群成员重新对应" },
      ...
      "group:message": { title: "跨年群消息", layout: "portrait-chat-thread", anomaly: "不同年份消息在同一宿舍群内重叠" },
      "private:incoming-call": { title: "同名私聊来电", layout: "portrait-call-thread", anomaly: "同一姓名从复制宿舍深处呼入" },
      "private:message": { title: "宋明与时间错位私聊", layout: "portrait-private-thread", anomaly: "未送达、未来时间戳或回到零点的消息" },
      "private:main": { title: "结局后的私聊", layout: "portrait-private-thread", anomaly: "头像、联系人或发送时间在离开四楼后仍留下不可逆变化" },
      "system:main": { title: "身份与秩序记录", layout: "portrait-system-record", anomaly: "公告、学籍与账号看似完整，却不能替代现场发生过的选择" },
      "system:documents": { title: "宿管文档柜", layout: "portrait-document-stack", anomaly: "三年合同、羽毛债和校长室路线分段显现" },
      "system:gallery": { title: "合照查看器", layout: "portrait-photo-viewer", anomaly: "林峰位置模糊并被相同面孔覆盖" },
      "system:incoming-calls": { title: "双林峰来电", layout: "portrait-call-panel", anomaly: "两个同名账号同时来电，反光中的头像慢半拍呼吸" },
      "system:members": { title: "学生名单载体", layout: "portrait-record-list", anomaly: "名单行、联系人和群成员互相覆盖" },
      "system:message": { title: "系统提示消息", layout: "portrait-system-message", anomaly: "用户不存在与楼层时间异常在手机内显示" },
      ...
      "system:system": { title: "账号状态面板", layout: "portrait-system-record", anomaly: "个人账号显示用户不存在或被同名账号取代" },
    },
    characters: {
