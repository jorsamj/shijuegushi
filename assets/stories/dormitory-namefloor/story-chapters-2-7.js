(function () {
  ...
  function line(nodeId, chapterId, scene, speaker, text, nextNodeId, extra = {}) {
    // Ending routes use the compact scene-first call form so their chapter ownership cannot drift.
    if (String(chapterId || "").startsWith("namefloor_ending_")) {
      add({ nodeId, chapterId: chapterIds[7], scene: chapterId, speaker: scene, text: speaker, nextNodeId: text, ...(nextNodeId || {}) });
      return;
    }
  ...
  add({
    ...
    speaker: "旁白",
    text: "时间短暂停住。名册、手机、羽毛与活人的记忆保留在当前状态。结局不会由最后一次点击决定，它只会回看这一夜已经留下的后果。",
    type: "chapter-ending",
    contentType: "narration",
    nextNodeId: "nf_end_resolve",
    chapterEnding: { title: "第七章结束", subtitle: "身份、名册、人格与队友站队已写入结局判定入口。", cta: "查看这一夜的结果" },
  });

  add({ nodeId: "nf_end_resolve", chapterId: chapterIds[7], scene: "namefloor_ending_resolve", speaker: "旁白", text: "楼梯间的钟停在 08:08。所有被保存的记录同时等待同一项结算。", type: "ending", contentType: "narration", resolveEnding: true, routeResolvedEnding: true });

  // Formal ending routes are intentionally separate from the resolver: the resolver only selects
  // a consequence already earned in chapters 1-7, then this chain presents that consequence.
  line("nf_e1_001", "namefloor_ending_breakfast", "旁白", "四楼从尽头开始塌陷。装订钉断开后，楼梯终于只剩向下的一层一层。", "nf_e1_002", { endingRouteId: "E1", effects: [{ type: "space-collapse", level: "heavy", durationMs: 1100 }] });
  line("nf_e1_002", "namefloor_ending_breakfast", "周朝阳", "记录还需要核对。但我不会再用记录否认你刚才做过的选择。", "nf_e1_003", { endingRouteId: "E1", contentType: "dialogue", visualCharacter: "周朝阳" });
  line("nf_e1_003", "namefloor_ending_breakfast", "谷雨", "林峰，别走太快。我还记得你答应过，出去以后请我吃早饭。", "nf_e1_004", { endingRouteId: "E1", contentType: "dialogue", visualCharacter: "谷雨", effects: [{ type: "name-restore", level: "medium", durationMs: 700 }] });
  add({ nodeId: "nf_e1_004", chapterId: chapterIds[7], scene: "namefloor_ending_breakfast", endingRouteId: "E1", speaker: "旁白", text: "手机先恢复了联系人，再恢复班群。合照里四个人的位置都在，黑色头像从原来的成员位淡了下去。", type: "phone-interaction", contentType: "phone-text", phoneScreen: { kind: "group", title: "班级群", time: "06:12", memberCount: 42, members: [{ name: "林峰" }, { name: "周朝阳" }, { name: "谷雨" }, { name: "宋明" }], systemNotice: "群成员与现场人数已核对。" }, nextNodeId: "nf_e1_005" });
  line("nf_e1_005", "namefloor_ending_breakfast", "宋明", "我还活着，先别把我当成一个结论。等天亮了，再慢慢问。", "nf_e1_006", { endingRouteId: "E1", contentType: "dialogue", visualCharacter: "宋明" });
  line("nf_e1_006", "namefloor_ending_breakfast", "旁白", "清晨的小店刚开门。每个人都累得说不出话，却没有一个人把另一个人留在身后。", "nf_e1_007", { endingRouteId: "E1", effects: [{ type: "dawn-light", level: "medium", durationMs: 1000 }] });
  add({ nodeId: "nf_e1_007", chapterId: chapterIds[7], scene: "namefloor_ending_breakfast", endingRouteId: "E1", endingId: "E1", speaker: "谷雨", text: "林峰，走了，回宿舍。", type: "ending", contentType: "dialogue", visualCharacter: "谷雨", resolveEnding: true });

  line("nf_e2_001", "namefloor_ending_song_guard", "旁白", "出口只容得下前面的人。身后的走廊却还在把所有被删掉的名字往回拖。", "nf_e2_002", { endingRouteId: "E2", effects: [{ type: "space-collapse", level: "heavy", durationMs: 1000 }] });
  line("nf_e2_002", "namefloor_ending_song_guard", "同行宋明", "羽毛对我有反应，不是因为它认得原来的他。它只是在问，我现在愿意做什么。", "nf_e2_003", { endingRouteId: "E2", contentType: "dialogue", visualCharacter: "宋明", effects: [{ type: "feather-split", level: "medium", durationMs: 700 }] });
  line("nf_e2_003", "namefloor_ending_song_guard", "林峰", "一起走。我们还能再想办法。", "nf_e2_004", { endingRouteId: "E2", contentType: "dialogue", visualCharacter: "林峰" });
  line("nf_e2_004", "namefloor_ending_song_guard", "同行宋明", "不用替我证明我是谁。把他们带出去就行。", "nf_e2_005", { endingRouteId: "E2", contentType: "dialogue", visualCharacter: "宋明" });
  add({ nodeId: "nf_e2_005", chapterId: chapterIds[7], scene: "namefloor_ending_song_guard", endingRouteId: "E2", speaker: "旁白", text: "他把门关在追逐者和其他人之间。手机上，“宋明”的头像仍亮着，联系人名称却开始慢慢褪色。", type: "phone-interaction", contentType: "phone-text", phoneScreen: { kind: "private", title: "宋明", time: "06:21", messages: [{ sender: "宋明", text: "不用回头。" }], systemNotice: "账号将在离线后保留最后一条记录。" }, nextNodeId: "nf_e2_006" });
  line("nf_e2_006", "namefloor_ending_song_guard", "旁白", "门后的撞击声停下时，散页边缘多了一行不属于学校格式的手写名字。", "nf_e2_007", { endingRouteId: "E2", effects: [{ type: "door-seal", level: "heavy", durationMs: 900 }] });
  add({ nodeId: "nf_e2_007", chapterId: chapterIds[7], scene: "namefloor_ending_song_guard", endingRouteId: "E2", endingId: "E2", speaker: "同行宋明", text: "我不是宋明。可我记得，他不想让你们死。", type: "ending", contentType: "dialogue", visualCharacter: "宋明", resolveEnding: true });

  line("nf_e3_001", "namefloor_ending_guyu_voice", "旁白", "林峰的名字正在从门牌、聊天记录和每个人的嘴边一起消失。", "nf_e3_002", { endingRouteId: "E3", effects: [{ type: "name-glitch", level: "heavy", durationMs: 900 }] });
  line("nf_e3_002", "namefloor_ending_guyu_voice", "谷雨", "林峰。林峰，别停。", "nf_e3_003", { endingRouteId: "E3", contentType: "dialogue", visualCharacter: "谷雨" });
  line("nf_e3_003", "namefloor_ending_guyu_voice", "周朝阳", "每叫一次，名册就在改写。谷雨，停下，那一行在变成你。", "nf_e3_004", { endingRouteId: "E3", contentType: "dialogue", visualCharacter: "周朝阳" });
  line("nf_e3_004", "namefloor_ending_guyu_voice", "谷雨", "那就别让它白改。你记住他，我来把他送出去。", "nf_e3_005", { endingRouteId: "E3", contentType: "dialogue", visualCharacter: "谷雨" });
  add({ nodeId: "nf_e3_005", chapterId: chapterIds[7], scene: "namefloor_ending_guyu_voice", endingRouteId: "E3", speaker: "旁白", text: "谷雨的联系人名称先空了下来，接着头像、群成员和合照里的位置依次淡去。只有一段没有联系人名称的语音仍被保存在林峰手机里。", type: "phone-interaction", contentType: "phone-text", phoneScreen: { kind: "private", title: "无联系人", time: "06:33", messages: [{ sender: "", text: "峰哥，你别忘了你是谁。", voicePending: true }], systemNotice: "此为世界内语音记录，待后续配音合同接入。" }, nextNodeId: "nf_e3_006" });
  line("nf_e3_006", "namefloor_ending_guyu_voice", "旁白", "晨光里的人数没有少，只有林峰本能地知道，自己忘记了一个最不该忘记的人。", "nf_e3_007", { endingRouteId: "E3", effects: [{ type: "portrait-vacancy", level: "heavy", durationMs: 900 }] });
  add({ nodeId: "nf_e3_007", chapterId: chapterIds[7], scene: "namefloor_ending_guyu_voice", endingRouteId: "E3", endingId: "E3", speaker: "旁白", text: "峰哥，你别忘了你是谁。", type: "ending", contentType: "narration", resolveEnding: true });

  line("nf_e4_001", "namefloor_ending_betrayal_message", "旁白", "他们已经离开楼梯间，新的证据却在门后那一刻才恢复成完整的时间线。", "nf_e4_002", { endingRouteId: "E4", effects: [{ type: "signal-return", level: "medium", durationMs: 700 }] });
  line("nf_e4_002", "namefloor_ending_betrayal_message", "周朝阳", "原始记录和今晚的新记忆对上了。被留下的那个人，不是我们以为的东西。", "nf_e4_003", { endingRouteId: "E4", contentType: "dialogue", visualCharacter: "周朝阳" });
  line("nf_e4_003", "namefloor_ending_betrayal_message", "旁白", "没有人立刻责备林峰。沉默比任何一句话都更像一扇已经合死的门。", "nf_e4_004", { endingRouteId: "E4" });
  add({ nodeId: "nf_e4_004", chapterId: chapterIds[7], scene: "namefloor_ending_betrayal_message", endingRouteId: "E4", speaker: "旁白", text: "一个早已被删除的账号重新亮起。发送时间停在他们作出放逐决定之前。", type: "phone-interaction", contentType: "phone-text", phoneScreen: { kind: "private", title: "", time: "06:41", messages: [{ sender: "", text: "你明明说过相信我。", status: "发送时间：删除前" }], systemNotice: "账号状态：离线。" }, nextNodeId: "nf_e4_005" });
  ...
  const endingConditions = {
    ...
    E5: { required: ["维持固定人数", "确认到齐", "用原人补配额"], excluded: ["砸断名册配额", "E2-E4"] },
    E6: { required: ["合法另林获名", "玩家林无账号而离开"], excluded: ["未处理第五影子", "E3-E5"] },
    E7: { required: ["第五人种子", "放行额外影子", "身份保持未定"], excluded: ["出口已审计", "E3-E6"] },
    E1: { required: ["真宋明、林峰、周朝阳、谷雨均生还", "救援、砸断配额、出口审计"], excluded: ["核心人物死亡/失名", "固定人数仍生效", "未处理第五影子"] },
  };
