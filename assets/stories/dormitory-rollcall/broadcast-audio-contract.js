(function () {
  ...
  window.DORMITORY_BROADCAST_AUDIO_CONTRACT = {
    ...
    cues: [
      ...
      cue("dorm_broadcast_unlisted_person", ["dorm_chapter_05", "dorm_chapter_06"], ["dorm_05_003", "dorm_06_001"], "请未登记人员，前往值班室。", "制度化、无威胁表演", 4, "Use once per scene; stop within 120ms when the node changes."),
      cue("dorm_broadcast_deadline_0113", ["dorm_chapter_06"], ["dorm_06_001"], "距一点十三分，还有一分钟。请确认登记人数。", "克制提醒，不作倒计时鼓点", 5, "Stop within 120ms on node exit; no loop."),
      cue("dorm_broadcast_correction_prompt", ["dorm_chapter_06"], ["dorm_06_008"], "请提交最终名单。", "制度化、留白", 3, "Fade within 140ms when player chooses or leaves the node."),
      ...
      cue("dorm_broadcast_ending_a", ["ending"], ["dorm_ending_true_dawn"], "更正完成。所有姓名均已记录。", "平静、恢复秩序", 4, "Play once, then stop before ending report."),
      cue("dorm_broadcast_ending_b", ["ending"], ["dorm_ending_legal_count"], "四一七登记人数，四人。", "机械但不夸张", 4, "Play once, then stop before ending report."),
      cue("dorm_broadcast_ending_c", ["ending"], ["dorm_ending_left_behind"], "人数不匹配。请重新确认。", "平静、无惊吓", 4, "Play once, then stop before ending report."),
      cue("dorm_broadcast_ending_d", ["ending"], ["dorm_ending_broken_broadcast"], "下一次点名，将于零点十七分开始。", "克制循环感", 5, "Play once, then stop before ending report."),
    ],
  };
})();
