...
12. The rules broadcast and final corrected-rollcall broadcast are produced with the authorised 科大讯飞超拟人语音合成 WebAPI. Do not approve the story for public launch until their source permission, distribution permission, and headphone/desktop-speaker/phone-speaker listening records have been signed off.

Release gate: `node scripts/check-dormitory-release-gates.mjs` must remain failing until the XFYUN delivery source, distribution permission, three-device listening sign-off, cross-story save/load sign-off, and mobile playthrough sign-off are recorded in `audioProduction`. It is deliberately included in the production-readiness aggregate so a Draft build cannot be mistaken for a release candidate.
