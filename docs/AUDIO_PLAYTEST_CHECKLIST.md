...
12. The 14 dormitory institutional broadcasts are produced with the authorised Volcengine Doubao Voice Synthesis Model 2.0 HTTP unidirectional API. Do not approve the story for public launch until their source permission, distribution permission, cadence/order review, and headphone/desktop-speaker/phone-speaker listening records have been signed off.

Release gate: `node scripts/check-dormitory-release-gates.mjs` must remain failing until the Volcengine delivery source, distribution permission, three-device listening sign-off, 14-broadcast cadence sign-off, cross-story save/load sign-off, mobile playthrough sign-off, Rain Call regression, mobile background restore, and console no-error checks are recorded in `audioProduction`. It is deliberately included in the production-readiness aggregate so a Draft build cannot be mistaken for a release candidate.
