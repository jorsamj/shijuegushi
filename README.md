# 第二人生

## 项目定位

互动视觉小说体验平台 Demo。

欢迎来体验别人的人生。

## 当前阶段

前端 Demo 视觉资产增强阶段。

## 文档版本

- 软件 PRD：v0.6
- 《雨夜来电》故事 PRD：v0.3

## 当前开放故事

《雨夜来电》

## 当前文档

- [docs/PRD_SOFTWARE.md](docs/PRD_SOFTWARE.md)：软件产品 PRD，描述产品形态、页面结构、功能模块、交互规则、数据结构、存档机制和验收标准。
- [docs/PRD_STORY_雨夜来电.md](docs/PRD_STORY_雨夜来电.md)：故事剧情 PRD，描述故事设定、人物、章节、线索、推理链、选择分支和结局。
- [docs/STORY_SCRIPT_雨夜来电.md](docs/STORY_SCRIPT_雨夜来电.md)：《雨夜来电》剧情脚本文档，采用 6 个长章节结构，描述章节、节点、选项、线索、变量和结局节点。

## 本轮重构说明

- 将 12 个短章节调整为 6 个长章节。
- 将正式线索压缩为 6 条关键线索。
- 减少剧情过程中的强制弹窗。
- 强化悬疑张力、连续阅读体验和有效选择。
- 优化未解锁内容展示，避免大量问号占位。
- 降低旁白比例，提高对话、动作和当前冲突比例。

## 本轮视觉增强

- 新增 `assets/visual-assets.js` 统一维护场景、角色、线索、章节封面和道具视觉配置。
- 游戏页根据 `scene` 显示雨夜出租屋、猫眼门口、旧手机、照片放大等视觉资产。
- 线索库、人物关系面板、章节结算和结局报告接入图标、剪影和证据链视觉。
- 视觉资产全部使用 CSS / HTML / SVG 式结构，不依赖外链图片。

## 运行说明

可直接双击打开 [index.html](index.html) 体验前端 Demo。

也可以在仓库根目录启动本地静态服务：

```bash
python -m http.server 8080
```

然后访问：

```text
http://localhost:8080
```

## 数据校验

```bash
node scripts/validate-story-data.mjs
```

## 下一阶段

根据体验反馈继续完善视觉小说交互、剧情节奏与前端细节。
